...
namespace AiAssertions.OpenAi.Clients;

/// <summary>
/// OpenAI implementation of the AIAssert model and tool-calling client abstractions.
/// </summary>
internal sealed class OpenAiClient : IToolCallingClient
{
    ...
    /// </summary>
    /// <param name="httpClient">The HTTP client used to call OpenAI.</param>
    /// <param name="options">The OpenAI client options.</param>
    internal OpenAiClient(HttpClient httpClient, OpenAiOptions options)
    {
    ...
    private async Task<JsonNode> SendAsync(JsonObject body, CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
            throw new HttpRequestException($"OpenAI endpoint returned {(int)response.StatusCode}: {content}");
    ...
    private static string GetModelId(OpenAiModel model) =>
        model switch
        {
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(model), model, "Unknown OpenAI model.")
        };
}
