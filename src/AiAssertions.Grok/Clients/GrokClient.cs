...
using AiAssertions.Grok.Configuration;

namespace AiAssertions.Grok.Clients;

/// <summary>
/// Grok implementation of the AIAssert model and tool-calling client abstractions.
/// </summary>
internal sealed class GrokClient : IToolCallingClient
{
    ...
    /// </summary>
    /// <param name="httpClient">The HTTP client used to call xAI.</param>
    /// <param name="options">The Grok client options.</param>
    internal GrokClient(HttpClient httpClient, GrokOptions options)
    {
    ...
    private async Task<JsonNode> SendAsync(JsonObject body, CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
            throw new HttpRequestException($"Grok endpoint returned {(int)response.StatusCode}: {content}");
    ...
    private static string GetModelId(GrokModel model) =>
        model switch
        {
            GrokModel.Latest => "grok-latest",
            GrokModel.Grok4 => "grok-4",
            GrokModel.Grok3 => "grok-3",
            GrokModel.Grok3Mini => "grok-3-mini",
            _ => throw new ArgumentOutOfRangeException(nameof(model), model, "Unknown Grok model.")
        };
}
